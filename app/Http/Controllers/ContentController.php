...
class ContentController extends Controller
{
    public function update_hero(Request $request)
    {
        $rules = [
            ...
            'hero_image' => 'nullable|max:5120|mimes:jpg,jpeg,png', // Max 5MB image file
            'self_resume' => 'nullable|max:5120', // Max 5MB image file
        ...
        foreach ($contents as $content) {
            if ($content->key === 'hero_image' && $request->hasFile('hero_image')) {
                $filePath = $request->file('hero_image')->store('images');
                if ($content->value) {
            ...
            if ($content->key === 'self_resume' && $request->hasFile('self_resume')) {
                $filePath = $request->file('self_resume')->store('self_resume');
                if ($content->value) {
                    Storage::delete($content->value);
                }
                $content->value = $filePath;
